...
You can pass it a key and only that is removed (i.e. instead of the entire cache).
    ...
    my $get_asset_file_cache = $lh->flush_cache('get_asset_file');  # only delete the cached data for get_asset_file()
    
    my $entire_cache = $lh->flush_cache();# delete the entire cache
...
All output() methods support embedded methods: sub(), sup(), chr(), and numf().

Note: sub(), sup(), and numf() are simplified in that they only work with one argument.
...
The main keys are 'html' and 'plain'. Their values are the string to use in conjuction with the context's rendering of the value. Embedded arguments are supported in those values:
...
This was not called 'span' or 'inline' to avoid tieing it to terms of the underlying systems that one context uses.

=item * segment()

Same as fragment() except, in HTML context, it uses a div instead of span. (Think of "fragment" as small and arbitrary but "segment" as a more defined block.)
...
When we get real world examples of this I'll update the POD. For now you probably really want output,fragment or output,sub or output,sup.

This was not called 'div', 'block', or 'inline_block' to avoid tieing it to terms of the underlying systems that one context uses.
...
The idea is to embed ones that will likely not change and hopefully add to the meaning of the string.
    ...
    Your hair [output,fragment,is on fire,class,urgent]!
    
After that list (or instead of it) a single hashref can be passed in. The intent here is to be able to do any arbitrary name/value that the caller might want to use but is likely to change and/or ads little meaning if any to the string.
...
Document Locale::Maketext::Utils::Phrase::Norm, add more normalizing classes.
